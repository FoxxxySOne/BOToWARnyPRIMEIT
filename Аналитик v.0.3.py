...
def load_excel_data(file_path):
    try:
        ...
        required_columns = ['Лог. маршрут', '№', 'ФИО Водителя', 'Дата открытия', 
        ...
        DATA['Процент доставки возвратов'] = (
            DATA['Доставлено_возвратов'] / DATA['Всего_возвратов']
        ) * 100
...
# Анализ маршрута по парковке
def analyze_parking(parking_number):
    if DATA is None or PARKING_MAPPING is None:
        return "❌ Данные не загружены. Сначала скачайте файл."
    ...
    message = (
        ...
        parking_number, route_id, total_sum, total_fines, avg_items, total_routes, routes_without_returns,
        total_returns, delivered_returns, delivery_rate_returns,
        delivery_rate_boxes, avg_route_sum, net_profit
    )
    return message

# Навигация по путевым листам
async def show_all_routes(update: Update, context: ContextTypes.DEFAULT_TYPE, page=0):
    ...
    message = f"📋 <b>Список путевых листов (Страница {page + 1}/{total_pages}):</b>"
    await update.callback_query.message.reply_text(message, reply_markup=reply_markup, parse_mode="HTML")

# Обработка навигации по путевым листам
async def handle_all_routes_pagination(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    data = query.data.split('_')
    page = int(data[-1])
    await show_all_routes(update, context, page)

# Навигация по водителям
async def show_drivers_list(update: Update, context: ContextTypes.DEFAULT_TYPE, page=0):
    ...
    if nav_buttons:
        keyboard.append(nav_buttons)
    reply_markup = InlineKeyboardMarkup(keyboard)
    message = f"👥 <b>Список водителей (Страница {page + 1}/{total_pages}):</b>"
    await update.callback_query.message.reply_text(message, reply_markup=reply_markup, parse_mode="HTML")

# Обработка навигации по водителям
async def handle_drivers_pagination(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    data = query.data.split('_')
    page = int(data[-1])
    await show_drivers_list(update, context, page)
...
async def handle_route_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = (
        ...
        row['№'], row['Лог. маршрут'][:5], row['ФИО Водителя'], row['Дата открытия'],
...
async def choose_parking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Преобразуем unique_routes в pandas Series и очищаем от лишних пробелов
        unique_routes = pd.Series(unique_routes).astype(str).str.strip()
        print(f"Уникальные маршруты из 'Лог. маршрут': {unique_routes.tolist()}")
        # Преобразуем route_id в parking_mapping к строковому типу и удаляем лишние пробелы
        PARKING_MAPPING['route_id'] = PARKING_MAPPING['route_id'].astype(str).str.strip()
        print(f"Route IDs в parking_mapping: {PARKING_MAPPING['route_id'].unique().tolist()}")
        # Фильтруем parking_mapping по route_id, которые есть в unique_routes
        filtered_parking = PARKING_MAPPING[PARKING_MAPPING['route_id'].isin(unique_routes)]
        print(f"Отфильтрованные парковки: {filtered_parking}")
        ...
        if filtered_parking.empty:
            ...
            return
        # Создаем кнопки только для фильтрованных парковок
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton("📥 Выгрузить Путевые листы за предыдущий день", callback_data="download_previous_day")],
        [InlineKeyboardButton("📋 Все путевые листы", callback_data="show_all_routes")],
        [InlineKeyboardButton("👥 Список водителей", callback_data="show_drivers_list")],
        [InlineKeyboardButton("📍 Выбрать парковку", callback_data="choose_parking")]
    ]
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "download_previous_day":
        ...
        await handle_all_routes_pagination(update, context)
    elif query.data == "show_drivers_list":
        await show_drivers_list(update, context)
    elif query.data.startswith("drivers_page_"):
        await handle_drivers_pagination(update, context)
...
if __name__ == "__main__":
    ...
    app.add_handler(CallbackQueryHandler(handle_all_routes_pagination, pattern="^all_routes_page_"))
    app.add_handler(CallbackQueryHandler(show_drivers_list, pattern="^show_drivers_list$"))
    app.add_handler(CallbackQueryHandler(handle_drivers_pagination, pattern="^drivers_page_"))
